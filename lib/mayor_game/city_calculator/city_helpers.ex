defmodule MayorGame.CityHelpers do
  ...
  def calculate_city_stats(
    ...
    ordered_buildables_flat =
      Enum.flat_map(ordered_buildables, fn {_, v} -> v end)
  ...
  defp check_workers(%{} = reqs, citizens, required_count, education_diff) do
    Enum.filter(citizens, fn cit -> (cit.education >= reqs.workers.level && cit.education <= reqs.workers.level + education_diff) end) |> Enum.take(required_count)
  end
  ...
  defp activation_rounds_recursive(result_blob, buildables, city, season, education_diff) do
    ...
    if result_blob.updated_buildable_count > count do
      ...
      result_blob_input = result_blob |> Map.put(:result_buildables, elem(buildables_split_by_jobs, 1))
      activation_rounds_recursive(result_blob_input, elem(buildables_split_by_jobs, 0), city, season, education_diff + 1)
    else
      result_blob
    end
  end
      ...
      fn individual_buildable, acc ->
        ...
        if individual_buildable.requires == nil do
          ...
          if checked_reqs == [] do
            ...
            if Map.has_key?(individual_buildable.requires, :workers) && individual_buildable.jobs != 0 do
              ...
              required_worker_count = if individual_buildable.jobs == nil do individual_buildable.requires.workers.count else individual_buildable.jobs end
              checked_workers = check_workers(individual_buildable.requires, acc.citizens, required_worker_count, education_diff)
